...
//                        }
    ...
    stage ('Update Manifest'){
           ...
           steps {
                sh "echo $BUILD_ID"
sh "awk -v build_id=${env.BUILD_ID} '/image: himanshukingstorm\\/todo-app-py:v/ {sub(/v\\$/, "v" build_id)} {print}' todo_app_deployment.yml > temp.yaml && mv temp.yaml todo_app_deployment.yml"
